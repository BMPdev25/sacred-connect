...
/**
 * Priest NotificationCenter — displays all priest-role notifications with
 * read/unread state and tap-to-navigate for booking-type notifications.
 */
export default function NotificationCenter(): React.JSX.Element {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const data = await fetchNotifications('priest');
      setNotifications(data);
    } catch {
      // silent — badge will still display last known count
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleMarkAll = async () => {
    await markAllRead('priest');
    setNotifications((prev) => prev.map((n) => ({ ...n, read: true })));
  };

  const handlePress = async (item: AppNotification) => {
    if (!item.read) {
      await markAsRead(item._id, 'priest').catch(() => {});
      setNotifications((prev) =>
        prev.map((n) => (n._id === item._id ? { ...n, read: true } : n))
      );
    }
    // Navigate to the related booking if present
    if (item.relatedId && item.type === 'booking') {
      router.push({
        pathname: '/priest/(screens)/PriestBookingDetails' as any,
