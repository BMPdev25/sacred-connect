...
const EnhancedEarningsScreen = ({ navigation }) => {
  ...
  const renderEarningCard = (title, amount, subtitle, icon, color, change) => (
  ...
  const monthlyData = {
    labels: earningsData.monthlyTrends.map(trend => 
      new Date(2024, trend.month - 1).toLocaleDateString('en', { month: 'short' })
    ),
    datasets: [{
      data: earningsData.monthlyTrends.map(trend => trend.earnings || 0),
      color: (opacity = 1) => `rgba(255, 107, 107, ${opacity})`,
      strokeWidth: 2,
    }],
  };

  const categoryData = earningsData.categoryBreakdown.map((item, index) => ({
    name: item.category,
...
export default EnhancedEarningsScreen;
