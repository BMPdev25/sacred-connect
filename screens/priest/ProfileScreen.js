...
const HEADER_TOP_PADDING = Platform.OS === 'android' ? (Platform.constants && Platform.constants.StatusBarHeight) || 24 : 44;
...
const ProfileScreen = ({ navigation }) => {
  ...
  const calculateProfileCompletion = () => {
    ...
    if (userInfo.profile?.experience) completed++;
    if (userInfo.profile?.religiousTradition) completed++;
    if (userInfo.profile?.ceremonies && userInfo.profile.ceremonies.length > 0) completed++;
      ...
      <View style={[styles.header, { paddingTop: HEADER_TOP_PADDING }]}>
        <View style={styles.headerContent}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('Help')}>
            <Ionicons name="help-circle-outline" size={24} color={APP_COLORS.white} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 32 }}>
        <View style={styles.profileHeader}>
          ...
          <Text style={styles.userRole}>
            Senior Priest • {userInfo?.profile?.experience || 12} years experience
          </Text>
